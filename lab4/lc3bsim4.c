...
int main(int argc, char *argv[]) {                             
   ...
   // char *file_names[] = {"ucode4","aa.obj"};
   // initialize(file_names,1);
...
int getVectorAdder(){
   ...
   switch (VECTOR_MUX)
   {
       ...
       if((Low16bits(CURRENT_LATCHES.PSR) >> 15) == 1 && protectionChecker()){
           return CURRENT_LATCHES.EXCV = 0x02;
           break;
       }
       // unaligned
...
void drive_bus() {
   if(CURRENT_LATCHES.MICROINSTRUCTION[GATE_MARMUX]){
        // protection 
        if(CURRENT_LATCHES.STATE_NUMBER == 47 || CURRENT_LATCHES.STATE_NUMBER == 6 || CURRENT_LATCHES.STATE_NUMBER == 7 || CURRENT_LATCHES.STATE_NUMBER == 2 || CURRENT_LATCHES.STATE_NUMBER == 3){
       ...
       if(CURRENT_LATCHES.STATE_NUMBER == 47 || CURRENT_LATCHES.STATE_NUMBER == 6 || CURRENT_LATCHES.STATE_NUMBER == 7){
               if(newBus % 2){
                   unalignedFlag = 1;
               }
       }
